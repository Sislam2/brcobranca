  ...
  module Boleto
    module Template
      ...
      module Rghost
        ...
        # Gera o boleto em usando o formato desejado [:pdf, :jpg, :tif, :png, :ps, :laserjet, ... etc] Com o padrão do arrecadação de taxa para prefeituras
        #
        # @return [Stream]
        def to_febrabam(formato, options = {})
          modelo_arrecadacao_taxa(self, options.merge!(formato: formato))
        end
        ...
        def modelo_arrecadacao_taxa(boleto, options = {})
          doc = Document.new paper: :A4 # 210x297

          template_path = File.join(File.dirname(__FILE__), '..', '..', 'arquivos', 'templates', 'modelo_arrecadacao_taxa.eps')

          fail 'Não foi possível encontrar o template. Verifique o caminho' unless File.exist?(template_path)

          modelo_generico_template(doc, boleto, template_path)
          modelo_arrecadacao_taxa_cabecalho(doc, boleto)
          modelo_arrecadacao_taxa_rodape(doc, boleto)
          # Gerando codigo de barra com rghost_barcode
          doc.barcode_interleaved2of5(boleto.codigo_barras, width: '10.3 cm', height: '1.3 cm', x: '0.7 cm', y: '5.8 cm') if boleto.codigo_barras
